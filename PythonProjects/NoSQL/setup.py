...
def parse_message(data):
    '''Return a tuple containing the command, the key, and (optionally) the value cast to the approriate type.'''
...
def update_stats(command, success):
    '''Update the STATS dict with info about if executing *command* was a *succcess*.'''
    if success:
...
def handle_get(key):
    ...
    if key not in DATA:
        return (False, 'EROR: Key [{}] not found'.format(key))
    else:
...
def handle_append(key, value):
    '''Return a tuple containing True if the key's value could be appened to and the message to send back to the client.'''
...
def handle_stats():
    '''Retuen a tuple containing Tre and the contents of the STATS dict.'''
